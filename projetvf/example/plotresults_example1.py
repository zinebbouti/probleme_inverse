...
# ---------------------------------------------------------
# 1) Chargement simple des données
# ---------------------------------------------------------
...
# ---------------------------------------------------------
#  Solveur exact du problème de Riemann
# ---------------------------------------------------------
...
def exact_solution_sod(x, t, gamma=1.4):
    """Solution exacte du problème de Sod (utilise le solveur exact)."""
    rho_L, u_L, p_L = 1.0, 0.0, 1.0
    rho_R, u_R, p_R = 0.125, 0.0, 0.1
    return exact_riemann_solver(rho_L, u_L, p_L, rho_R, u_R, p_R, x, t, gamma)

# ---------------------------------------------------------
# 2) Figure : comparaison temporelle (3 sous-graphiques)
# ---------------------------------------------------------
def plot_comparison(t, x, rho, u, p, output_dir):
    # Solution exacte au temps final
    rho_exact, u_exact, p_exact = exact_solution_sod(x, t[-1])
    
    fig, axs = plt.subplots(1, 3, figsize=(14, 4))
    fig.suptitle("Évolution du tube à choc de Sod", fontsize=16)
    times = [0, len(t)//2, len(t)-1]
    vars = [(rho, rho_exact, "Densité ρ"),
            (u, u_exact, "Vitesse u"),
            (p, p_exact, "Pression p")]
    
    for ax, (var, var_exact, label) in zip(axs, vars):
        for k in times:
            ax.plot(x, var[k], label=f"t={t[k]:.2f}s")
        # Ajouter solution exacte
        ax.plot(x, var_exact, 'k--', linewidth=2, label='Exact')
        ax.set_xlabel("x")
        ax.set_ylabel(label)
        ax.legend()
        ax.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "comparaison.png"), dpi=200)
    plt.close()
def plot_sod_comparison(t, x, rho, u, p, output_dir):
    """
    Figure style article pour le problème de Sod
    avec une seule sous-figure montrant WENO-RF-3
    """
    tf = -1  # temps final
    time_final = t[tf]
    
    # Solution exacte
    rho_exact, u_exact, p_exact = exact_solution_sod(x, time_final)
    
    # Créer la figure
    fig, ax = plt.subplots(figsize=(6, 5))
    
    # Titre global avec paramètres
    fig.suptitle(f'DENSITY    t={time_final:.2f}  cfl=0.6  n={len(x)}', 
                 fontsize=11, y=0.98)
    
    # Tracer la solution numérique (cercles vides)
    ax.plot(x, rho[tf], 'o', markersize=4, markerfacecolor='none', 
            markeredgecolor='black', markeredgewidth=0.8, label='WENO-RF-3')
    ...
    # Label de la sous-figure
    ax.text(0.02, 0.98, '(a)', transform=ax.transAxes,
            fontsize=11, verticalalignment='top', fontweight='bold')
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "sod_weno3_comparison.png"), 
                dpi=300, bbox_inches='tight')
    plt.close()
    print("  → sod_weno3_comparison.png créée")

# ---------------------------------------------------------
# 3) Figure : état final détaillé (3 courbes)
# ---------------------------------------------------------
def plot_final_state(t, x, rho, u, p, output_dir):
    tf = -1  # dernier instant
    
    # Solution exacte
    rho_exact, u_exact, p_exact = exact_solution_sod(x, t[tf])
    
    plt.figure(figsize=(10, 7))
    plt.suptitle(f"État final Sod (t={t[tf]:.2f}s)", fontsize=16)
    
    # DENSITÉ
    plt.subplot(3, 1, 1)
    plt.plot(x, rho[tf], 'b', label='WENO-3')
    plt.plot(x, rho_exact, 'k--', linewidth=2, label='Exact')
    plt.ylabel("ρ")
    plt.legend()
    plt.grid(True)
    ...
    # VITESSE
    plt.subplot(3, 1, 2)
    plt.plot(x, u[tf], 'r', label='WENO-3')
    ...
    # PRESSION
    plt.subplot(3, 1, 3)
    plt.plot(x, p[tf], 'orange', label='WENO-3')
    plt.plot(x, p_exact, 'k--', linewidth=2, label='Exact')
    plt.ylabel("p")
    plt.xlabel("x")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "etat_final.png"), dpi=200)
    plt.close()
...
def main():
    ...
    out = os.path.join(script_dir, "../resultats/example1")
    ...
    plot_comparison(t, x, rho, u, p, out)
    plot_final_state(t, x, rho, u, p, out)
    plot_sod_comparison(t, x, rho, u, p, out)
