program example1_euler_1d_lax
   ...
   myweno = weno(ncells=nc, k=3, eps=1e-6_rk,bc_type=BC_OUTFLOW)
...
contains
   ...
   subroutine output(action)
      ...
      character(*), parameter :: folder = "./resultats/lax/"
      ...
      select case(action)
      case(1)
         write (stdout, '(1x,a)') "Running Lax shock tube problem ..."
      ...
      case(3)
         ...
         write (stdout, '(1x,a)') "Results saved in ./resultats/lax/"
      end select
   end subroutine output

end program example1_euler_1d_lax
